...
export default function RoomBookingPage() {
  ...
  return (
    <>
      <PageHeader
        title={t('ui.room.page_title', 'Room Booking')}
        subtitle={t('ui.room.page_subtitle', 'Reserve a room at ICAI Bhawan, Nagpur')}
      />
      ...
      <section className="container" style={{ padding: '2.5rem 1rem' }}>
        <div className="room-booking-grid">
          ...
          <div>
            {/* Step 1 — room */}
            <div className="tiny-eyebrow">{t('ui.room.step1', 'Step 1 · Choose a room')}</div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="row gap-1 muted-text" style={{ fontSize: '.8125rem', marginTop: '.3rem' }}>
                      <IconUsers size="sm" /> {r.capacity} {t('ui.room.seats', 'seats')}
                    </div>
            ...
            {/* Step 2 — date */}
            <div className="tiny-eyebrow">{t('ui.room.step2', 'Step 2 · Pick a date')}</div>
                ...
                return (
                  ...
                  >
                    <span style={{ fontSize: '.6875rem', fontWeight: 700, opacity: .8 }}>
                      {i === 0 ? t('ui.room.today', 'TODAY') : WEEKDAYS[d.getDay()].toUpperCase()}
                    </span>
            ...
            <div className="row" style={{ justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: '.5rem' }}>
              <div className="tiny-eyebrow">{t('ui.room.step3', 'Step 3 · Available time slots')}</div>
              <div className="row gap-3" style={{ fontSize: '.75rem', color: 'var(--muted-foreground)' }}>
                <span className="row gap-1"><span className="avail-dot avail-free" /> {t('ui.room.available', 'Available')}</span>
                <span className="row gap-1"><span className="avail-dot avail-booked" /> {t('ui.room.booked', 'Booked')}</span>
              </div>
            </div>
            <div className="muted-text" style={{ fontSize: '.8125rem', marginTop: '.4rem' }}>
              {room.name} · {dateLabel} ·{' '}
              <strong style={{ color: 'var(--secondary)' }}>{availableCount} of {SLOTS.length} {t('ui.room.slots_free', 'slots free')}</strong>
            </div>
            <div className="slot-grid" style={{ marginTop: '.85rem' }}>
              {SLOTS.map((s, i) => {
                const isBooked = slotStatus(roomId, dateIdx, i) === 'booked';
                ...
                return (
                  <button
                    ...
                    className={'slot-chip' + (isBooked ? ' is-booked' : '') + (active ? ' is-active' : '')}
                  >
                    <IconClock size="sm" />
                    <span style={{ fontWeight: 700 }}>{s}</span>
                    <span style={{ fontSize: '.6875rem' }}>
                      {isBooked ? t('ui.room.booked', 'Booked') : t('ui.room.available', 'Available')}
                    </span>
                  </button>
                );
              })}
            ...
            <div className="card booking-summary">
              {confirmed ? (
                <div style={{ textAlign: 'center' }}>
                  ...
                  <h3 style={{ marginTop: '.75rem', fontSize: '1.125rem', fontWeight: 700 }}>
                    {t('ui.room.booking_confirmed', 'Booking confirmed')}
                  </h3>
                  <p className="muted-text" style={{ fontSize: '.8125rem', marginTop: '.35rem' }}>
                    {t('ui.room.sent_to', 'A confirmation has been sent to')} {confirmed.name}.
                  </p>
                  <div style={{ marginTop: '1rem', textAlign: 'left', fontSize: '.875rem', display: 'flex', flexDirection: 'column', gap: '.45rem' }}>
                    <div className="row" style={{ justifyContent: 'space-between' }}><span className="muted-text">{t('ui.room.room_label', 'Room')}</span><strong>{confirmed.room}</strong></div>
                    <div className="row" style={{ justifyContent: 'space-between' }}><span className="muted-text">{t('ui.room.date_label', 'Date')}</span><strong>{confirmed.date}</strong></div>
                    <div className="row" style={{ justifyContent: 'space-between' }}><span className="muted-text">{t('ui.room.time_label', 'Time')}</span><strong>{confirmed.slot}</strong></div>
                    <div className="row" style={{ justifyContent: 'space-between' }}><span className="muted-text">{t('ui.room.ref_label', 'Reference')}</span><strong>{confirmed.ref}</strong></div>
                  </div>
                  <button className="btn btn-outline" style={{ marginTop: '1.25rem', width: '100%', justifyContent: 'center' }} onClick={reset}>
                    {t('ui.room.book_another', 'Book another room')}
                  </button>
                </div>
              ) : (
                <form onSubmit={onConfirm}>
                  <h3 style={{ fontSize: '1.125rem', fontWeight: 700 }}>{t('ui.room.booking_summary', 'Booking summary')}</h3>
                  <div style={{ marginTop: '.75rem', display: 'flex', flexDirection: 'column', gap: '.45rem', fontSize: '.875rem' }}>
                    <div className="row" style={{ justifyContent: 'space-between' }}><span className="muted-text">{t('ui.room.room_label', 'Room')}</span><strong>{room.name}</strong></div>
                    <div className="row" style={{ justifyContent: 'space-between' }}><span className="muted-text">{t('ui.room.date_label', 'Date')}</span><strong>{dateLabel}</strong></div>
                    <div className="row" style={{ justifyContent: 'space-between' }}>
                      <span className="muted-text">{t('ui.room.time_label', 'Time')}</span>
                      <strong style={{ color: slotIdx === null ? 'var(--muted-foreground)' : 'inherit' }}>
                        {slotIdx === null ? t('ui.room.select_slot', 'Select a slot') : SLOTS[slotIdx]}
                      </strong>
                    </div>
                  </div>
                  <div style={{ borderTop: '1px solid var(--border)', margin: '1rem 0' }} />
                  <label className="field-label">{t('ui.room.name_label', 'Full name')}</label>
                  <input className="input-base" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Your name" />
                  <label className="field-label" style={{ marginTop: '.75rem' }}>{t('ui.room.email_label', 'Email')}</label>
                  ...
                  <label className="field-label" style={{ marginTop: '.75rem' }}>{t('ui.room.purpose_label', 'Purpose (optional)')}</label>
                  ...
                  >
                    {t('ui.room.confirm_booking', 'Confirm Booking')} <IconArrowRight size="sm" />
                  </button>
                  <p className="muted-text" style={{ fontSize: '.6875rem', marginTop: '.6rem', textAlign: 'center' }}>
                    {t('ui.room.demo_note', 'Demo booking — no real reservation is made.')}
                  </p>
                </form>
              )}
            </div>
